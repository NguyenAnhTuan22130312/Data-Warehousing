...
def load_to_datawarehouse(config, data_date):
    ...
    try:
        # ===== 8.1 Kết nối tới ControlManagementDB để ghi log bắt đầu =====
        control_conn = connect_mysql("databaseControlManagementDB", config)
        control_cur = control_conn.cursor()

        # Ghi log bắt đầu (RUNNING)
         # ===== 8.1.a Kiểm tra job Transform có thành công không =====
        control_cur.execute("""
            SELECT status 
            FROM log_history 
        ...
        # ===== 8.2 Kết nối tới Staging và Data Warehouse =====
        ...
        # ===== 8.3 Đọc dữ liệu staging theo ngày api_date =====
        ...
        # ===== 8.4 Kiểm tra / thêm mới dữ liệu vào dim_date =====
        ...
        if not dw_cur.fetchone():
            ...
            # ===== 8.4.1 Kiểm tra dữ liệu cũ cùng ngày =====
        ...
        # ===== 8.5 Load dữ liệu vào dim_player và fact_performance =====
        for p in players:
            # Kiểm tra player có tồn tại (is_current = 1)
            dw_cur.execute("SELECT * FROM dim_player WHERE player_id = %s AND is_current = 1", (p["player_id"],))
            current_player = dw_cur.fetchone()

            # 8.5.a Nếu chưa tồn tại → thêm mới dim_player
            if not current_player:
                ...
                # 8.5.b Nếu đã có → lấy player_key cũ
                player_key = current_player["player_key"]

            # 8.5.c Ghi dữ liệu fact_performance tương ứng player/date
            dw_cur.execute(
                """
        ...
        # ===== 8.8 Cập nhật log kết thúc và gửi email thông báo =====
        end_time = datetime.datetime.now()

        # Cập nhật log trong Control DB
...
if __name__ == "__main__":
    ...
    load_to_datawarehouse(config, today)
